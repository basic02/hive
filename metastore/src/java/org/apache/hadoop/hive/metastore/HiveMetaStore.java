...
public class HiveMetaStore extends ThriftHiveMetastore {
  ...
  public static class HMSHandler extends FacebookBase implements IHMSHandler, ThreadLocalRawStore {
    ...
    private boolean getIfServerFilterenabled() throws MetaException{
      boolean isEnabled = hiveConf.getBoolVar(ConfVars.METASTORE_SERVER_FILTER_ENABLED);
      ...
      String filterHookClassName = hiveConf.getVar(ConfVars.METASTORE_FILTER_HOOK);
    ...
    private MetaStoreFilterHook loadFilterHooks() throws IllegalStateException  {
      ...
      try {
        return (MetaStoreFilterHook)Class.forName(
            ...
            HiveConf.class).newInstance(hiveConf);
      } catch (Exception e) {
     ...
     */
    private void authorizeTableForPartitionMetadata(final String dbName, final String tblName)
        throws NoSuchObjectException, MetaException {

      FilterUtils.checkDbAndTableFilters(
          isServerFilterEnabled, filterHook, dbName, tblName);
    }
    ...
    private void drop_database_core(RawStore ms,
      ...
      try {
        ...
        // call internal function to avoid filter
        List<String> allTables = getMS().getAllTables(db.getName());
    ...
    */
    ...
    public List<Table> get_table_objects_by_name(final String dbName, final List<String> tableNames)
      ...
      try {
        ...
        // filter table names before fetching the table objects.
        distinctTableNames = FilterUtils.filterTableNamesIfEnabled(
            isServerFilterEnabled, filterHook, dbName, distinctTableNames);
        int startIndex = 0;
    ...
    public List<String> get_table_names_by_filter(
      ...
      try {
        ...
        tables = FilterUtils.filterTableNamesIfEnabled(
            isServerFilterEnabled, filterHook, dbName, tables);
      } catch (Exception e) {
        ex = e;
        if (e instanceof MetaException) {
          throw (MetaException) e;
    ...
    private List<Partition> add_partitions_core(final RawStore ms,
      ...
      try {
        ...
        for (final Partition part : parts) {
          ...
          if (validatePartition(part, tblName, dbName, partsToAdd, ms, ifNotExists,
              partitionKeys)) {
     ...
     * @throws TException
     */
    private boolean validatePartition(final Partition part, final String tblName, final String dbName, final Set<PartValEqWrapperLite> partsToAdd,
    ...
    public Partition get_partition(final String db_name, final String tbl_name,
      ...
      Exception ex = null;
      try {
        authorizeTableForPartitionMetadata(db_name, tbl_name);
    ...
    public List<Partition> get_partitions_with_auth(final String dbName,
      ...
      try {
        ...
        authorizeTableForPartitionMetadata(dbName, tblName);
    ...
    public List<PartitionSpec> get_partitions_pspec(final String db_name, final String tbl_name, final int max_parts)
      ...
      try {
        authorizeTableForPartitionMetadata(dbName, tbl_name);
        Table table = get_table_core(dbName, tableName);
        List<Partition> partitions = get_partitions(dbName, tableName, (short) max_parts);
        partitions = FilterUtils.filterPartitionsIfEnabled(isServerFilterEnabled,
            filterHook, partitions);
    ...
    public GetPartitionsResponse get_partitions_with_specs(GetPartitionsRequest request)
      ...
      Exception ex = null;
      try {
        authorizeTableForPartitionMetadata(dbName, tableName);
        Table table = get_table_core(dbName, tableName);
    ...
    public List<String> get_partition_names(final String db_name, final String tbl_name,
      ...
      try {
        ...
        ret = FilterUtils.filterPartitionNamesIfEnabled(isServerFilterEnabled,
            filterHook, db_name, tbl_name, ret);
      } catch (Exception e) {
        ex = e;
        if (e instanceof MetaException) {
          throw (MetaException) e;
    ...
    public List<FieldSchema> get_schema_with_environment_context(String db, String tableName,
      ...
      try {
        ...
        try {
          tbl = get_table_core(db, base_table_name);
          fireReadTablePreEvent(db, tableName);
        } catch (NoSuchObjectException e) {
          throw new UnknownTableException(e.getMessage());
        }
    ...
    public List<String> get_partition_names_ps(final String db_name,
      ...
      try {
        ...
        ret = FilterUtils.filterPartitionNamesIfEnabled(isServerFilterEnabled, filterHook,
            db_name, tbl_name, ret);
      } catch (Exception e) {
        ex = e;
        rethrowException(e);
      } finally {
    ...
    public Index get_index_by_name(final String dbName, final String tblName,
      ...
      try {
        ...
        ret = FilterUtils.filterIndexIfEnabled(isServerFilterEnabled, filterHook, ret);
      } catch (Exception e) {
        ex = e;
        rethrowException(e);
      } finally {
    ...
    public List<String> get_index_names(final String dbName, final String tblName,
      ...
      try {
        ...
        ret = FilterUtils.filterIndexNamesIfEnabled(
            isServerFilterEnabled, filterHook, dbName, tblName, ret);
      } catch (Exception e) {
        ex = e;
        if (e instanceof MetaException) {
          throw (MetaException) e;
    ...
    public List<Index> get_indexes(final String dbName, final String tblName,
      ...
      try {
        ...
        ret = FilterUtils.filterIndexesIfEnabled(isServerFilterEnabled, filterHook, ret);
      } catch (Exception e) {
        ex = e;
        rethrowException(e);
      } finally {
    ...
    private String getPartName(HiveObjectRef hiveObject) throws MetaException {
      ...
      if (partValue != null && partValue.size() > 0) {
        try {
          authorizeTableForPartitionMetadata(hiveObject.getDbName(), hiveObject.getDbName());
    ...
    public void alter_function(String dbName, String funcName, Function newFunc)
        throws InvalidOperationException, MetaException, TException {
      validateForAlterFunction(dbName, funcName, newFunc);
      boolean success = false;
      RawStore ms = getMS();
      try {
        ms.openTransaction();
